...
class LlamaShakespeareTrainer:
    def __init__(self, model_name="meta-llama/Llama-3.2-1B-Instruct"):
        ...
        self.dataloader = DataLoader(self.dataset, batch_size=4, shuffle=True)  # Increased batch size
        self.data_iter = iter(self.dataloader)
        
        # Optimizer with lower learning rate for full model training
        self.optimizer = torch.optim.AdamW(
            self.model.parameters(),
            lr=1e-5,  # Lower LR for full model training
            weight_decay=0.01
        )
    ...
    def train_step(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(batch_size):
                ...
                token_strings = [self.tokenizer.decode([tid], skip_special_tokens=False) 
